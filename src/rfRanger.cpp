...
rfRanger::rfRanger( const uint32_t m, const bool prediction_mode, std::unique_ptr<Data> data, const uint32_t numTrees, uint32_t mtry, uint32_t rfThrd, uint32_t seed ) :
		...
		memory_saving_splitting( true ),
		// importance_mode( IMP_NONE ),
		// importance_mode( IMP_GINI ),
		// importance_mode( IMP_PERM_BREIMAN ),
		// importance_mode( IMP_PERM_LIAW ),
		importance_mode( IMP_PERM_RAW ),
		// importance_mode( IMP_GINI_CORRECTED ),
		splitrule( LOGRANK ),
		// splitrule( AUC ),
...
void rfRanger::saveImportance(size_t nPart, std::string forestDirname) {
	forest->output_prefix = forestDirname + std::string("/") + std::to_string(nPart) + std::string(".out");
